...
def load_pretrained(
    ...
    if num_classes == 1000 and cfg["num_classes"] == 1001:
        ...
        classifier_name + ".weight" in state_dict
        and num_classes != state_dict[classifier_name + ".weight"].size(0)
        and pre_num == 0
    ):
        print(
            "Removing the last fully connected layer due to dimensions mismatch ("
            + str(num_classes)
            + " != "
            + str(state_dict[classifier_name + ".weight"].size(0))
            + ").",
            flush=True,
        )
        ...
        strict = False
    elif "head_cls.weight" in state_dict:
        print(
            "Removing head_cls due to dimensions mismatch ("
            + str(num_classes)
            + " != "
            + str(state_dict["head_cls.weight"].size(0))
            + ").",
            flush=True,
        )
        # completely discard fully connected for all other differences between pretrained and created model
        del state_dict["head_cls.weight"]
        del state_dict["head_cls.bias"]
        strict = False
