...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 ...
                 drop_path_rate=0.1, hybrid_backbone=None, norm_layer=nn.LayerNorm, num_frames=8, attention_type='divided_space_time', dropout=0., label_emb = '', mlp=0,text_model = '', lp=False,num_seg=0,extra_tr='',normp=False,drope=0., cfg=None):
        super().__init__()
        self.attention_type = attention_type
        self.depth = depth
        ...
        if not label_emb == '' and text_model=='':
            ...
            self.head = nn.Linear(embed_dim, self.label_emb.shape[1]) if num_classes > 0 else nn.Identity()
            if mlp==3:
                self.text_mlp = nn.Sequential(nn.Linear(self.label_emb.shape[1],self.label_emb.shape[1]),
                                              nn.ReLU(),
        ...
        if '-space' in extra_tr and num_seg > 0:
            ...
            if hasattr(cfg.MODEL, 'RET_HEAD') and cfg.MODEL.RET_HEAD > 0:
                ...
                if extra_tr[0] == '1':
                    ...
                    Block(
                        dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale,
                        drop=drope, attn_drop=attn_drop_rate, drop_path=dpr[-1], norm_layer=norm_layer, attention_type='space_only'), 
                    ...
                    nn.ModuleList([Block(
                        dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale,
                        drop=drope, attn_drop=attn_drop_rate, drop_path=dpr[-(int(extra_tr[0])-i)], norm_layer=norm_layer, attention_type='space_only') for i in range(int(extra_tr[0]))]), 
                    norm_layer(embed_dim)
                    ]) 
                ...
                if hasattr(cfg.MODEL, 'RET_POS') and cfg.MODEL.RET_POS:
                    self.head_ret_pos_embed = nn.Parameter(torch.zeros(1, num_seg*self.ret, embed_dim))   
    ...
    def no_weight_decay(self):
        return {'pos_embed', 'cls_token', 'time_embed','head_pos_embed','head_ret_pos_embed'}
    ...
    def forward(self, x):
        ...
        if hasattr(self, 'num_seg') and self.num_seg>0:
            ...
            if hasattr(self, 'ret') and self.ret > 0: 
                ...
                if hasattr(self, 'next') and self.next:
                    step_emb = F.embedding((video_f.topk(k=self.ret,dim=1)[1]+1).clip(max=video_f.shape[-1]-1), weight = self.head.weight)
                else:    
                    step_emb = F.embedding(video_f.topk(k=self.ret,dim=1)[1], weight = self.head.weight)
                num_s = step_emb.shape[0]
                if hasattr(self, 'head_ret_pos_embed'):
                    if hasattr(self, 'ret_pos_mul') and self.ret_pos_mul:
                        step_emb = step_emb.view(step_emb.shape[0]//self.num_seg, -1, 768) * self.head_ret_pos_embed
                    else:    
                        step_emb = step_emb.view(step_emb.shape[0]//self.num_seg, -1, 768) + self.head_ret_pos_embed
                x = torch.cat((x.view(-1, self.num_seg, 768), step_emb.view(num_s//self.num_seg, -1, 768)), 1)
                if self.extra_tr == 1:
                ...
                if self.extra_tr == 1:
                    x = self.head[0](x.view(-1, self.num_seg, 768), x.shape[0],self.num_seg,1)
                else:
                    for i in range(self.extra_tr):
        ...
        if isinstance(self.label_emb,torch.Tensor) and len(self.text) > 0 and self.training:
            if hasattr(self, 'text_model'):
                text_emb = self.text_model(text)['sentence_embedding']
                text_pred = F.linear(F.normalize(text_emb,1)*4.5, F.normalize(self.label_emb.to(x.device),1)*4.5)
                return x, text_pred  
            else:
                return x, None
        ...
        if hasattr(self, 'step2task'):
            x = (x.unsqueeze(1)*(x.unsqueeze(1)==x.topk(k=5,dim=1)[0].unsqueeze(2)).float()).sum(1)
...
class vit_base_patch16_224(nn.Module):
    def __init__(self, cfg, **kwargs):
        ...
        self.model = VisionTransformer(img_size=cfg.DATA.TRAIN_CROP_SIZE, num_classes=cfg.MODEL.NUM_CLASSES, patch_size=patch_size, embed_dim=768, depth=12, num_heads=12, mlp_ratio=4, qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6), drop_rate=0., attn_drop_rate=0., drop_path_rate=dpr, num_frames=cfg.DATA.NUM_FRAMES, attention_type=cfg.TIMESFORMER.ATTENTION_TYPE, label_emb = emb, mlp = mlp, text_model = cfg.MODEL.TEXT_MODEL, lp=lp,num_seg=num_seg,extra_tr=extra,drope=drope,cfg=cfg,**kwargs)
        ...
        if self.pretrained:
            load_pretrained(self.model, num_classes=self.model.num_classes, in_chans=kwargs.get('in_chans', 3), filter_fn=_conv_filter, img_size=cfg.DATA.TRAIN_CROP_SIZE, num_patches=self.num_patches, attention_type=self.attention_type, pretrained_model=pretrained_model, num_frames=cfg.DATA.NUM_FRAMES, pre_num=cfg.MODEL.PRE_CLASSES)
        else:
