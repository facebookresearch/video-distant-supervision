...
class Howto100m(torch.utils.data.Dataset):
    ...
    def __init__(self, cfg, mode, num_retries=20):
        ...
        if len(cfg.TRAIN.TEXT) > 0:    
            ...
            self.tokenizer = AutoTokenizer.from_pretrained('sentence-transformers/paraphrase-mpnet-base-v2') if cfg.MODEL.TEXT_MODEL == 'paraphrase-mpnet-base-v2' else None
        ...
        with PathManager.open(path_to_file, "r") as f:
            for clip_idx, path_label in enumerate(f.read().splitlines()):
                assert (
                    ...
                    == 5 or len(path_label.split(self.cfg.DATA.PATH_LABEL_SEPARATOR))
                    == 6
                )
    ...
    def __getitem__(self, index):
        ...
        for i_try in range(self._num_retries):
            ...
            words = None
            text=None
            if (start == None or type(self.caps) == type(' ')) and not (self.caps==None and self.labels==None):
                ...
                if start ==None:
                    ...
                    for i in range(len(cap)):
                        temp.append(check_time(start, end, cap['start'].values[i], cap['end'].values[i]))
                    ind =  np.argmax(temp)
                ...
                if hasattr(self, 'min_len') and self.min_len > 0:    
                    ...
                    while len(q.split(' ')) < self.min_len:
                        if ind - mi>0 and isinstance(cap['text'].values[ind-mi],str):
                            q = cap['text'].values[ind-mi]+ ' ' +q
                            s = cap['start'].values[ind-mi]
                        if ind + mi<len(cap) and isinstance(cap['text'].values[ind+mi],str):
                ...
                if self.sample < 1 or (not 'train' in self.mode):    
                    ...
                    if not type(sen) == type(' ') or len(sen) == 0:
                        sen = ' '
                    text = self.tokenizer.encode_plus(sen, max_length=self.max_len,
                    padding='max_length',
                    truncation=True,
