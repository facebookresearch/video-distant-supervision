...
def perform_test(test_loader, model, test_meter, cfg, writer=None):
    ...
    for cur_iter, (inputs, labels, video_idx, meta) in enumerate(test_loader):
        ...
        if cfg.DETECTION.ENABLE:
            ...
            if cfg.TRAIN.LABEL_EMB == '' and not cfg.TRAIN.TEXT == '':
                meta = {k:meta[k].view(-1, meta[k].shape[-1]) for k in meta}
                pred = model([inputs, meta])
                vid_emb = pred[0]
                text_emb = pred[1]
                if du.dist.get_world_size() > 1:
                    vid_emb = allgather(vid_emb)
                    text_emb = allgather(text_emb)
                preds = F.linear(vid_emb, weight=text_emb)
            ...
            # Gather all the predictions across all the devices to perform ensemble.
            if cfg.NUM_GPUS > 1 and not ( cfg.TRAIN.LABEL_EMB == '' and not cfg.TRAIN.TEXT == ''):
...
def test(cfg):
    ...
    #if len(cfg.TEST.CHECKPOINT_FILE_PATH) >0:
